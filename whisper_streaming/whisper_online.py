...
@lru_cache
def load_audio(data, samplerate):
    a, _ = librosa.load(data, sr=samplerate)
    return a

def load_audio_chunk(data, samplerate, beg, end):
    # audio = load_audio(data, samplerate)
    beg_s = int(beg*samplerate)
    end_s = int(end*samplerate)
    return data[beg_s:end_s]
