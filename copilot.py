...
msgs = ["Михаил, спасибо за вашу честность. Я понимаю, что у вас возникли определенные предубеждения по отношению к нашей продукции. Это нормально, когда люди скептически относятся к чему-то новому или к продуктам, о которых слышали смешанные отзывы. Я был бы рад поделиться с вами актуальной информацией о наших пылесосах и тех инновациях, которые мы внедрили в модель Kirby 9000. Это поможет вам сформировать свое мнение на основе фактов. Может быть, есть конкретные моменты или вопросы, которые вызывают у вас сомнения? Это позволит мне дать более развернутый и точный ответ.",
...
В ответе формулируй конкретное высказывание для менеджера применительно к текущему моменту диалога с клиентом. Учитывай контекст и эмоциональный окрас беседы. Цель одна - "заполучить клиента". Каждый твой ответ - лишь ответ как человека - без ненужной информации или вводных слов модели перед настоящим ответом.
    ...
    def start_server(self):
        ...
        while True:
            try:
                print("trying to connect from copilot.py: start")
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as self.s:
                    print("trying to connect from copilot.py: inside with")
                    self.s.connect((self.host, self.port))
                    print(f"Connected by {self.s}")
                    self.receiver()
            except ConnectionRefusedError as e:
                print("SSH connection is broken, reconnecting in 1 sec")
                time.sleep(1)
            except Exception as e:
